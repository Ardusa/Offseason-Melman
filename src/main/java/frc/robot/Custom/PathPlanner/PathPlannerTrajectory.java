package frc.robot.Custom.PathPlanner;
...
public class PathPlannerTrajectory extends Trajectory {
  ...
  public PathPlannerTrajectory(
      ...
      boolean reversed,
      boolean fromGUI) {
    super(generatePath(pathPoints, constraints.maxVelocity, constraints.maxPositiveAcceleration, constraints.maxNegativeAcceleration, reversed));
  ...
  private static List<State> generatePath(
      List<Waypoint> pathPoints, double maxVel, double maxPosAccel, double maxNegAccel, boolean reversed) {
    ...
    for (List<Waypoint> splitPath : splitPaths) {
      ...
      calculateMaxVel(joined, maxVel, maxPosAccel, maxNegAccel, shouldReverse);
      calculateVelocity(joined, splitPath, maxPosAccel, maxNegAccel);
  ...
  private static void calculateMaxVel(
      List<PathPlannerState> states, double maxVel, double maxPosAccel, double maxNegAccel, boolean reversed) {
  ...
  private static void calculateVelocity(
      List<PathPlannerState> states, List<Waypoint> pathPoints, double maxPosAccel, double maxNegAccel) {
    ...
    for (int i = 1; i < states.size(); i++) {
      ...
      if (deltaPos > 0) {
        double vMax = Math.sqrt(Math.abs(Math.pow(v0, 2) + (2 * maxPosAccel * deltaPos)));
        states.get(i).velocityMetersPerSecond =
    ...
    for (int i = states.size() - 2; i > 1; i--) {
      ...
      double vMax = Math.sqrt(Math.abs(v0 * v0 + 2 * maxNegAccel * deltaPos));
  ...
  /** Horizontal View */
  public Translation2d getField() {
    return new Translation2d(FIELD_LENGTH_METERS, FIELD_WIDTH_METERS);
  }
